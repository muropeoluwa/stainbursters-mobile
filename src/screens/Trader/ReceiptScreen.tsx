...
const ReceiptScreen = ({ route }) => {
  ...
  const { customerName, phone, items, totalAmount, amountPaid, balanceDue, timestamp, traderName } = orderDetails;
    ...
    const itemRows = items.map(
      item =>
        `<tr>
          ...
          <td style="padding: 6px; text-align: right;">₦${item.price * item.quantity}</td>
        </tr>`
    ).join('');
        ...
        <body>
          <div class="card">
            ...
            <div class="section">
              ...
              <div class="balance">Balance Due: ₦${balanceDue}</div>
            </div>
    ...
    <ScrollView style={styles.container}>
      <View style={styles.card}>
        ...
        <View style={styles.section}>
          ...
          <Text>Paid: ₦{amountPaid}</Text>
          <Text style={styles.balance}>Balance Due: ₦{balanceDue}</Text>
        </View>
